export const navigation = [
  {
    title: 'Doc',
    links: [{ title: 'Document Reference', href: '/' }],
  },

  // {
  //   title: 'AHU dan Chiller 601',
  //   links: [
  //     {
  //       title: 'Overview',
  //       href: '/docs/ahu-chiller-601/overview',
  //     },
  //   ],
  // },

  {
    title: 'AHU dan Chiller Railink',
    links: [
      {
        title: 'Overview',
        href: '/docs/ahu-chiller-railink/overview',
      },
    ],
  },

  {
    title: 'AC Shelter Kalayang T1',
    links: [
      {
        title: 'System',
        href: '/docs/ac-shelter-kalayang-t1/system',
      },
      {
        title: 'Indoor Unit',
        href: '/docs/ac-shelter-kalayang-t1/indoor',
      },
      {
        title: 'Outdoor Unit',
        href: '/docs/ac-shelter-kalayang-t1/outdoor',
      },
    ],
  },

  {
    title: 'AC Shelter Kalayang T2',
    links: [
      {
        title: 'System',
        href: '/docs/ac-shelter-kalayang-t2/system',
      },
      {
        title: 'Indoor Unit',
        href: '/docs/ac-shelter-kalayang-t2/indoor',
      },
      {
        title: 'Outdoor Unit',
        href: '/docs/ac-shelter-kalayang-t2/outdoor',
      },
    ],
  },

  {
    title: 'AC Shelter Kalayang T3',
    links: [
      {
        title: 'System',
        href: '/docs/ac-shelter-kalayang-t3/system',
      },
      {
        title: 'Indoor Unit',
        href: '/docs/ac-shelter-kalayang-t3/indoor',
      },
      {
        title: 'Outdoor Unit',
        href: '/docs/ac-shelter-kalayang-t3/outdoor',
      },
    ],
  },
  // {
  //   title: 'HVAC Troubleshooting',
  //   links: [
  //     {
  //       title: 'Overview',
  //       href: '/docs/hvac-troubleshooting/overview',
  //     },
  //   ],
  // },

  {
    title: 'AC Shelter Kalayang TIB',
    links: [
      {
        title: 'System',
        href: '/docs/ac-shelter-kalayang-tib/system',
      },
      {
        title: 'Indoor Unit',
        href: '/docs/ac-shelter-kalayang-tib/indoor',
      },
      {
        title: 'Outdoor Unit',
        href: '/docs/ac-shelter-kalayang-tib/outdoor',
      },
    ],
  },
  // {
  //   title: 'Escalator',
  //   links: [
  //     {
  //       title: 'Overview',
  //       href: '/docs/escalator/overview',
  //     },
  //     {
  //       title: 'Guide',
  //       href: '/docs/escalator/guide',
  //     },
  //     {
  //       title: 'Note',
  //       href: '/docs/escalator/note',
  //     },
  //   ],
  // },
  // {
  //   title: 'Elevator',
  //   links: [
  //     {
  //       title: 'Overview',
  //       href: '/docs/elevator/overview',
  //     },
  //     {
  //       title: 'Guide',
  //       href: '/docs/elevator/guide',
  //     },
  //     {
  //       title: 'Note',
  //       href: '/docs/elevator/note',
  //     },
  //   ],
  // },
  // {
  //   title: 'ATD (Automatic Door)',
  //   links: [
  //     {
  //       title: 'BFT',
  //       href: '/docs/atd/bft',
  //     },
  //     {
  //       title: 'Dorma',
  //       href: '/docs/atd/dorma',
  //     },
  //     {
  //       title: 'Geze',
  //       href: '/docs/atd/geze',
  //     },
  //     {
  //       title: 'Solic',
  //       href: '/docs/atd/solic',
  //     },
  //     {
  //       title: 'Tormax',
  //       href: '/docs/atd/tormax',
  //     },
  //     {
  //       title: 'UMS',
  //       href: '/docs/atd/ums',
  //     },
  //   ],
  // },

  // {
  //   title: 'Technical Notes',
  //   links: [
  //     { title: 'Kontaktor', href: '/docs/technical-notes/kontaktor' },
  //     { title: 'Star-Delta', href: '/docs/technical-notes/star-delta' },
  //     {
  //       title: 'VRV IV',
  //       href: '/docs/technical-notes/vrv-iv',
  //     },
  //   ],
  // },
]
